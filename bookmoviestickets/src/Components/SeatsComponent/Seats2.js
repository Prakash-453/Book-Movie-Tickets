...
//     rows.reduce(
//       (total, row) =>
//         total + (row === "A" || row === "I" ? colsWithoutGap : colsWithGap),
//       0
//     );
...
const Seats2 = () => {
  ...
  const colsWithGap = 28;
  const colsWithoutGap = 30;
  ...
  const handlePay = () => {
    const convenienceFee = 18 * selectedSeats.length;
    const gst = 0.18 * (totalPrice + convenienceFee);
    const grandTotal = totalPrice + convenienceFee + gst;
  ...
  // ✅ Fixed useEffect (rows is now stable)
  useEffect(() => {
