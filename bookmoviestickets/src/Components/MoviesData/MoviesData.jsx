...
let hasStoredMovies = false; // Persistent flag outside the component
...
export default function StoreMovies() {
  ...
  const storeMoviesInFirebase = async () => {
    ...
    try {
      ...
      for (const movie of movies) {
        // Query Firestore to check if the movie already exists
        const q = query(collectionRef, where("title", "==", movie.title));
        const querySnapshot = await getDocs(q);

        if (querySnapshot.empty) {
          // If no existing movie found, add the new movie
  ...
  useEffect(() => {
    if (!hasStoredMovies) {
      hasStoredMovies = true; // Set the flag to true to prevent re-execution
